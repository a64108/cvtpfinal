...
def run(cap):
    ...
    gamemode = 1  # Set gamemode to 1 when in Option 1
    ...
    while True:
        ...
        if hands_landmarks:
            for hand_landmarks, hand_handedness in zip(hands_landmarks, results.multi_handedness):
                ...
                # Detect OK gesture using GestosJogo's method
                is_ok = gestos_jogo.gesture_library.detect_ok_gesture(hand_landmarks)

                # Detect hand state using GestosJogo's method
                hand_state = gestos_jogo.gesture_library.detect_hand_state(hand_landmarks)

                if hand_type == "Left":
                    left_ok = is_ok
                    if hand_state == 0:
                        print("Left Hand: Nothing happening")
                    elif hand_state == 1:
                    ...
                    right_ok = is_ok
                    if hand_state == 0:
                        print("Right Hand: Nothing happening")
                    elif hand_state == 1:
        ...
        # Display "Modo Teste" at the top middle of the frame
